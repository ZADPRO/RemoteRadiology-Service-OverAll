...
func GetNotificationCountController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		Status, Message, Count := service.GetNotificationCountService(dbConn, int(idValue.(float64)))
...
func NotificationController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.GetRequestBody[model.ViewNotificationReq](c, true)
		if !ok {
			return
		}
		...
		resVal := service.NotificationService(dbConn, data, int(idValue.(float64)))

		payload := map[string]interface{}{
			"status":     resVal.Status,
			"message":    resVal.Message,
			"data":       resVal.Data,
			"totalcount": resVal.TotalCount,
		}
...
func ReadStatusController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.GetRequestBody[model.ReadStatusReq](c, true)
		if !ok {
			return
		}
		...
		resVal := service.ReadStatusService(dbConn, data, int(idValue.(float64)))
