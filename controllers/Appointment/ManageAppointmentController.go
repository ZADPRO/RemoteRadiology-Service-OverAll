...
func ViewPatientHistory() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		historyData, ConsultantStatus, ConsultantLink := service.ViewPatientHistoryService(dbConn, int(idValue.(float64)))
...
func ListAuditLogController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		Data := service.ListAuditLogService(dbConn)
