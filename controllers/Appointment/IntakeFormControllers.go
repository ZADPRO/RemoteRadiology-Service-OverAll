...
func ViewPatientController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		ViewIntakeData := service.ViewPatientService(dbConn, *data)
...
func GetConsentDataController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		resVal := service.GetConsentDataService(dbConn, data)
...
func AllowOverrideController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//Request Should Be Encrypt
		data, ok := helper.GetRequestBody[model.GetOverRideReportReq](c, true)
		if !ok {
			return
		}
		...
		resValStatus, resValMessage := service.AllowOverrideService(dbConn, data, int(idValue.(float64)))

		payload := map[string]interface{}{
			"status":  resValStatus,
			"message": resValMessage,
		}
