...
func GetAllPatientController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.GetAllPatientReq](c)
		if !ok {
			return
		}
		...
		RadiologistData := service.GetAllPatientService(dbConn, *data)
...
func GetPatientDataController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.GetPatientReq](c)
		if !ok {
			return
		}
		...
		RadiologistData := service.GetPatientDataService(dbConn, *data, int(idValue.(float64)))
