...
func PostAddReceptionistController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.ReceptionistRegisterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PostReceptionistService(dbConn, *data, int(idValue.(float64)))
...
func PatchReceptionistController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.RequestHandler[model.UpdateReceptionistReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PatchReceptionistService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
