...
func PostCoDoctorController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		status, message := service.PostCoDoctorService(dbConn, *data, int(idValue.(float64)))
...
func PatchCoDoctorController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		status, message := service.PatchCoDoctorService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
