...
func PostScanCenterController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.ScanCenterRegisterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PostScanCenterService(dbConn, *data, int(idValue.(float64)))
...
func PatchScanCenterController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.RequestHandler[model.UpdateScanCentertReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PatchScanCenterService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
