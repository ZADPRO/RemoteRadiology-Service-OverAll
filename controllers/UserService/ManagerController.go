...
func PostManagerController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.ManagerRegisterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PostManagerService(dbConn, *data, int(idValue.(float64)))
...
func PatchManagerController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.RequestHandler[model.UpdateManagerReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PatchManagerService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
