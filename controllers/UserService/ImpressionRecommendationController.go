...
func GetFooterReportController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		status, message, data := service.GetFooterReportService(dbConn)
		token := accesstoken.CreateToken(idValue, roleIdValue)
...
func SaveFooterReportController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.SaveReportFooterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.SaveFooterReportService(dbConn, *data)
		token := accesstoken.CreateToken(idValue, roleIdValue)
