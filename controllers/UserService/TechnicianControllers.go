...
func PostAddTechnicianController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.TechnicianRegisterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PostTechnicianService(dbConn, *data, int(idValue.(float64)))
...
func PatchUpdateTechnicianController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.RequestHandler[model.UpdateTechnicianReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PatchTechnicianService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
