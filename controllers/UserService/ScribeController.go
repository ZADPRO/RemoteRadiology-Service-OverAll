...
func PostScribeController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		data, ok := helper.RequestHandler[model.ScribeRegisterReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PostScribeService(dbConn, *data, int(idValue.(float64)))
...
func PatchScribeController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		data, ok := helper.RequestHandler[model.UpdateScribeReq](c)
		if !ok {
			return
		}
		...
		status, message := service.PatchScribeService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
