...
func PostWellgreenPerformingProviderController() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		status, message := service.PostWellgreenPerformingProviderService(dbConn, *data, int(idValue.(float64)))
...
func PatchWellgreenPerformingProviderController() gin.HandlerFunc {
	return func(c *gin.Context) {
		idValue, idExists := c.Get("id")
		roleIdValue, roleIdExists := c.Get("roleId")
		...
		status, message := service.PatchWellgreenPerformingProviderService(dbConn, *data, int(idValue.(float64)))
		token := accesstoken.CreateToken(idValue, roleIdValue)
