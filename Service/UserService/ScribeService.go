...
func PostScribeService(db *gorm.DB, reqVal model.ScribeRegisterReq, idValue int) (bool, string) {
	...
	err := db.Raw(query.GetUsersCountSQL, 7).Scan(&TotalCount).Error
	if err != nil {
		log.Printf("ERROR: Failed to fetch User Total Count: %v", err)
		return false, "Something went wrong, Try Again"
	}

	CustId := "WGS" + strconv.Itoa(TotalCount[0].TotalCount+1)
	...
	ScribeDomain := model.CreateScribeDomainModel{
		UserId:         int(UserData.UserId),
	...
	ScribeDomainerr := tx.Create(&ScribeDomain).Error
	if ScribeDomainerr != nil {
...
func PatchScribeService(db *gorm.DB, reqVal model.UpdateScribeReq, idValue int) (bool, string) {
	log := logger.InitLogger()
	...
	PreviousData := model.GetAllScribeData{}

	errPrev := tx.Raw(query.GetAllScribeDataSQL, reqVal.ID).Scan(&PreviousData).Error
	if errPrev != nil {
	...
	ScribeDomainerr := tx.Exec(
		query.UpdateScribeDomainSQL,
	...
	if ScribeDomainerr != nil {
		log.Printf("ERROR: Failed to update Scribe Domain: %v\n", ScribeDomainerr)
		tx.Rollback()
		return false, "Something went wrong, Try Again"
	}
