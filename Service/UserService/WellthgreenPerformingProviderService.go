...
func PostWellgreenPerformingProviderService(db *gorm.DB, reqVal model.RadiologistRegisterReq, idValue int) (bool, string) {
	...
	err := db.Raw(query.GetUsersCountSQL, 10).Scan(&TotalCount).Error
	if err != nil {
		log.Printf("ERROR: Failed to fetch User Total Count: %v", err)
		return false, "Something went wrong, Try Again"
	}

	CustId := "WGPP" + strconv.Itoa(TotalCount[0].TotalCount+1)
	...
	RadiologistDomain := model.CreateWGPPDomainModel{
		UserId:           int(UserData.UserId),
...
func PatchWellgreenPerformingProviderService(db *gorm.DB, reqVal model.UpdateRadiologistReq, idValue int) (bool, string) {
	log := logger.InitLogger()
	...
	RadiologistDomainerr := tx.Exec(
		query.WGPPUpdateRadiologistDomainSQL,
