...
func PostDoctorService(db *gorm.DB, reqVal model.DoctorRegisterReq, idValue int) (bool, string) {
	...
	err := db.Raw(query.GetUsersScanCountSQL, 5, reqVal.ScanCenterId).Scan(&TotalCount).Error
	if err != nil {
	...
	fmt.Print(reqVal.DigitalSignature)
	fmt.Print(reqVal.DriversLicenseNo)

	DomainData := model.CreateDoctorDomainModel{
		UserId:           int(UserData.UserId),
...
func PatchDoctorService(db *gorm.DB, reqVal model.UpdateDoctorReq, idValue int) (bool, string) {
	log := logger.InitLogger()
	...
	PreviousData := model.GetAllDoctorData{}

	errPrev := tx.Raw(query.GetAllDoctorDataSQL, reqVal.ID).Scan(&PreviousData).Error
	if errPrev != nil {
